...
"gprj V B a \<equiv> if B \<subseteq> d a \<and> B \<in> opens (space V) then (B, Presheaf.ar (presheaf V) $ (Space.make_inclusion (space V) B (d a)) $$ (e a)) else undefined"

definition gext :: "('A,'a) OVA \<Rightarrow> 'A Open \<Rightarrow> ('A, 'a) Valuation \<Rightarrow> ('A, 'a) Valuation" where
"gext V A b \<equiv> if d b \<subseteq> A \<and> A \<in> opens (space V) then (comb V (neut V A) b) else undefined"
...
definition valid :: "('A, 'a) OVA \<Rightarrow> bool" where
  "valid V \<equiv>
    let
        ...
        neutral_law_left = (\<forall>A a. A \<in> opens T \<longrightarrow> a \<in> elems \<longrightarrow> d a = A \<longrightarrow> mul (\<epsilon> A) a = a);
        neutral_law_right = (\<forall>A a. A \<in> opens T \<and> a \<in> elems \<longrightarrow> d a = A \<longrightarrow> mul a (\<epsilon> A) = a);
...
lemma validI :
  ...
  assumes neutral_law_left : "( \<forall> A a . A \<in> opens T \<longrightarrow> a \<in> elem \<longrightarrow> d a = A \<longrightarrow> mul (\<epsilon> A) a = a)"
  assumes neutral_law_right : "(\<forall> A a .A \<in> opens T \<and> a \<in> elem \<longrightarrow> d a = A \<longrightarrow> mul a (\<epsilon> A) = a)"
...
lemma valid_neutral_law_left  :
  fixes V :: "('A,'a) OVA"
  shows "valid V \<Longrightarrow> let \<epsilon> = neut V; T = space V; mul = comb V; elems = elems V in
    \<forall>A a. A \<in> opens T \<longrightarrow> a \<in> elems \<longrightarrow> d a = A \<longrightarrow> mul (\<epsilon> A) a = a"
  by (simp add: valid_def Let_def)

lemma valid_neutral_law_right  :
  fixes V :: "('A,'a) OVA"
  shows "valid V \<Longrightarrow> let  \<epsilon> = neut V; T = space V; mul = comb V; elems = elems V in
    \<forall>A a. A \<in> opens T \<and> a \<in> elems \<longrightarrow> d a = A \<longrightarrow> mul a (\<epsilon> A) = a"
  by (simp add: valid_def Let_def)
...
lemma neutral_element : "valid V \<Longrightarrow> A \<in> Space.opens (space V) \<Longrightarrow> d (neut V A) = A "
  by (simp add: d_def neut_def)

lemma neutral_is_element :
fixes V :: "('A,'a) OVA" and A :: "'A Open"
defines "\<epsilon>A \<equiv> neut V A"
assumes "valid V" and "A \<in> Space.opens (space V)"
shows "neut V A \<in> elems V"
proof -
  have "valid V"
    using assms(2) by blast
  moreover have "A \<in> Space.opens (space V)"
    by (simp add: assms(3))
  moreover have "Poset.valid_map  (PresheafMap.nat (neutral V) $ A)"
...
proof -
  ...
  moreover have "A \<in> opens (space V)" by fact
...
lemma d_neut : "valid V \<Longrightarrow> A \<in> opens (space V) \<Longrightarrow> \<epsilon>A = neut V A \<Longrightarrow> d \<epsilon>A = A"
  by (simp add: d_def neut_def)

lemma d_gprj : "valid V \<Longrightarrow>  a \<in> elems V \<Longrightarrow>  B \<in> opens (space V)
\<Longrightarrow> A \<in> opens (space V)\<Longrightarrow> B \<subseteq> A  \<Longrightarrow> d a = A \<Longrightarrow>  a_B = gprj V B a 
\<Longrightarrow> d a_B = B"
  by (simp add: d_def gprj_def)

lemma d_gext : "valid V \<Longrightarrow>  b \<in> elems V \<Longrightarrow>  B \<in> opens (space V)
\<Longrightarrow> A \<in> opens (space V)\<Longrightarrow> B \<subseteq> A  \<Longrightarrow> d b = B \<Longrightarrow>  b__A = gext V A b 
...
lemma global_inclusion_element : "valid V \<Longrightarrow> A \<in> Space.opens (space V) 
...
lemma le_imp_gle : "valid V \<Longrightarrow> A \<in> opens (space V) \<Longrightarrow> a1 \<in> local_elems V A
...
lemma le_imp_gle2 : "valid V \<Longrightarrow> A \<in> opens (space V) \<Longrightarrow> a1 \<in> elems V
...
lemma gle_imp_le : "valid V \<Longrightarrow> A \<in> opens (space V) \<Longrightarrow> a1 \<in> elems V \<Longrightarrow> d a1 = A \<Longrightarrow> d a2 = A
...
proof -
  ...
  moreover have "i \<in> inclusions (space V) \<and> Space.dom i = B \<and> Space.cod i = A"
    by (metis (mono_tags, lifting) Inclusion.select_convs(1) Inclusion.select_convs(2) Inclusion.select_convs(3) OVA.space_def OVA.valid_welldefined Presheaf.valid_welldefined assms(4) assms(5) assms(6) assms(7) d_def i_def inclusions_def make_inclusion_def mem_Collect_eq valid_make_inclusion valid_V) 
  moreover have "Poset.le \<Phi>B (\<Phi>i $$ (e a1)) (\<Phi>i $$ (e a2))" using Presheaf.prj_monotone
    by (metis OVA.space_def OVA.valid_welldefined \<Phi>A_def \<Phi>B_def \<Phi>i_def assms(10) assms(7) assms(9) calculation(3) calculation(4) local_inclusion_element valid_V) 
  ultimately show ?thesis
    by (metis OVA.space_def OVA.valid_welldefined \<Phi>i_def assms(10) assms(4) assms(5) assms(7) assms(8) assms(9) d_def gprj_def i_def image l_def le_def le_imp_gle local_elems_def local_inclusion_element pr_def valid_V)
qed
...
lemma stability:
  ...
  assumes valid_V: "valid V"
  assumes "B \<subseteq> A" and "B \<in> opens (space V)" and "A \<in> opens (space V)"
...
proof -
  ...
  moreover have "i \<in> Space.inclusions (space V)"
    by (metis (mono_tags, lifting) Inclusion.select_convs(1) OVA.space_def OVA.valid_welldefined assms(2) assms(3) assms(4) i_def inclusions_def make_inclusion_def mem_Collect_eq space_valid valid_make_inclusion valid_V) 
...
moreover have "() \<in> Poset.el (Poset.dom  (ar one $ i))" using Presheaf.terminal_value
  by (metis (full_types) OVA.valid_welldefined Presheaf.Presheaf.select_convs(1) Presheaf.valid_map_welldefined UNIV_unit UNIV_witness calculation(1) calculation(4) dom_proj old.unit.exhaust terminal_def valid_inclusion_cod valid_V) 
moreover have "((f $ B) \<cdot> (ar one $ i)) $$ () = ((f $ B) $$ ((ar one $ i)) $$ ())"
  ...
  moreover have "((Presheaf.ar (presheaf V) $ i) \<cdot> (f $ A)) $$ ()=  e \<epsilon>B"
    by (metis OVA.valid_welldefined \<epsilon>B_def calculation(10) calculation(3) calculation(4) calculation(8) e_def f_def neut_def one_def snd_conv valid_map_naturality valid_V) 
  moreover have "e \<epsilon>A=   (f $ A) $$ ()"
    by (simp add: \<epsilon>A_def e_def f_def neut_def)  
  ultimately show ?thesis
    by (smt (verit) OVA.space_def OVA.valid_welldefined Presheaf.valid_map_welldefined UNIV_unit UNIV_witness \<epsilon>A_def \<epsilon>B_def assms(2) assms(3) assms(4) compose_app dom_proj f_def gprj_def i_def neut_def neutral_element old.unit.exhaust poset_maps_valid terminal_value valid_map_naturality valid_V) 
qed

(* [Remark 3 cont., CVA] *)
lemma local_mono_imp_global : "valid V \<Longrightarrow> A \<in> opens (space V) \<Longrightarrow>  a1 \<in> local_elems V A \<Longrightarrow>  a1' \<in> local_elems V A
...
(* [Remark 3 cont., CVA] *)
lemma global_mono_imp_local : "valid V \<Longrightarrow> A \<in> opens (space V) 
...
lemma id_le_gprj :
  ...
  assumes "valid V"
  and  "A \<in> Space.opens (space V)" and "B \<in> Space.opens (space V)"  and "B \<subseteq> A"
...
proof -
  ...
  moreover have "Space.cod i = A \<and> Space.dom i = B \<and> i \<in> Space.inclusions (space V)"
...
lemma gprj_elem : "valid V \<Longrightarrow>  a \<in> elems V \<Longrightarrow>  B \<in> opens (space V)
\<Longrightarrow> A \<in> opens (space V)\<Longrightarrow> B \<subseteq> A  \<Longrightarrow> d a = A \<Longrightarrow>  a_B = gprj V B a \<Longrightarrow> a_B \<in> elems V"
...
lemma gext_elem :
  ...
  assumes "valid V"
  and  "b \<in> elems V" and "B \<in> Space.opens (space V)" and "A \<in> Space.opens (space V)"
  and  "B \<subseteq> A" and "B \<in> Space.opens (space V)" and "A \<in> Space.opens (space V)" and "d b = B"
...
proof -
  have "valid V"
    by (simp add: assms(1)) 
  moreover have "B \<subseteq> A \<and> B \<in> Space.opens (space V) \<and> A \<in> Space.opens (space V) \<and> d b = B"
...
proof -
  ...
  moreover have "d a_B = B"
    using a_B_def assms(5) assms(6) assms(7) assms(8) assms(9) calculation(1) d_gprj by blast
...
proof -
  ...
  moreover have "b__A = comb V (neut V A) b"
    by (simp add: b__A_def assms(5) assms(7) assms(8) gext_def) 
...
proof -
  ...
  moreover have "Poset.valid (\<Phi>0 A)"
    by (metis A_def OVA.space_def OVA.valid_welldefined \<Phi>0_def \<Phi>_def elems local_inclusion_domain posets_valid valid_V)
...
theorem strongly_neutral_cVriance :
  ...
  and  "B \<subseteq> A" and "B \<in> Space.opens (space V)" and "A \<in> Space.opens (space V)"
defines "ex \<equiv> gext V"
