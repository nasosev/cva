...
lemma iter0_le_suc_iter :   
  fixes P :: "'a Poset" and f :: "('a, 'a) PosetMap" and a :: "'a" and n  :: "nat"
  assumes P_complete : "is_complete P" 
  and a_el : "a \<in> el P" and f_valid : "valid_map f" and f_endo : "dom f = P \<and> cod f = P"
shows "le P (iter f 0 \<star> bot P) (iter f n \<star> bot P)"
  by (metis P_complete Poset.fun_app2 bot_as_inf bot_min cod_iter complete_equiv_cocomplete dom_iter dual_order.refl f_endo f_valid inf_el iter_valid iter_zero_app)  

lemma iter_trans:
  fixes P :: "'a Poset" and f :: "('a, 'a) PosetMap" and a :: "'a" and n m k :: "nat"
  assumes P_complete : "is_complete P" 
  and a_el : "a \<in> el P" and f_valid : "valid_map f" and f_endo : "dom f = P \<and> cod f = P"
  and nm: "le P (iter f n \<star> bot P) (iter f m \<star> bot P)"
  and mk: "le P (iter f m \<star> bot P) (iter f k \<star> bot P)"
shows "le P (iter f n \<star> bot P) (iter f k \<star> bot P)"
  by (smt (verit, del_insts) P_complete Poset.fun_app2 bot_as_inf cod_iter dom_iter dual_order.refl f_endo f_valid is_complete_def iter_valid mk nm valid_transitivity)
...
proof (induct rule: less_Suc_induct[where P = "\<lambda>n m. le P (iter f n \<star> bot P) (iter f m \<star> bot P)"])
  show "\<And> i. le P (iter f i \<star> bot P) (iter f (Suc i) \<star> bot P)"
    using iter_le_suc_iter
    by (metis P_complete a_el f_endo f_valid) 
next
  show "\<And>i j k. i < j \<Longrightarrow> j < k \<Longrightarrow> le P (iter f i \<star> bot P) (iter f j \<star> bot P) \<Longrightarrow> le P (iter f j \<star> bot P) (iter f k \<star> bot P) \<Longrightarrow> le P (iter f i \<star> bot P) (iter f k \<star> bot P)"
    using iter_trans
    by (smt (verit, ccfv_threshold) P_complete a_el f_endo f_valid) 
qed
...
lemma iter_join :   
  ...
  assumes P_complete : "is_complete P" 
  and a_el : "a \<in> el P" and  f_valid : "valid_map f" and f_endo : "dom f = P \<and> cod f = P"
shows "join P (iter f n \<star> bot P) (iter f m \<star> bot P) = iter f (max n m) \<star> bot P" 
proof -
  have "n = m \<Longrightarrow> join P (iter f n \<star> bot P) (iter f m \<star> bot P) =  iter f (max n m) \<star> bot P"
    by (metis P_complete bot_as_inf complete_equiv_cocomplete dual_order.refl f_endo f_valid inf_el iter_el join_idem max.idem)
  moreover have "n < m \<Longrightarrow> join P (iter f n \<star> bot P) (iter f m \<star> bot P) = iter f (max n m) \<star> bot P"
    by (metis P_complete bot_as_inf complete_equiv_cocomplete dual_order.refl f_endo f_valid inf_el iter_el iter_le_iter join_le max.absorb4) 
  moreover have "m < n \<Longrightarrow> join P (iter f n \<star> bot P) (iter f m \<star> bot P) = iter f (max n m) \<star> bot P"
    by (metis P_complete bot_as_inf complete_equiv_cocomplete dual_order.refl f_endo f_valid inf_el iter_el iter_le_iter join_comm join_le max.absorb3)
  ultimately show ?thesis
    by fastforce
qed
...
lemma iter_directed :   
  fixes P :: "'a Poset" and f :: "('a, 'a) PosetMap"
  assumes P_complete : "is_complete P" 
  and f_valid : "valid_map f" and f_endo : "dom f = P \<and> cod f = P" and f_cont : "is_continuous f"
shows "is_directed P { iter f n \<star> bot P | n . n \<in> UNIV }"
  unfolding is_directed_def
proof (safe, goal_cases)
  case (1 x n)
  then show ?case
...
next
  case (3 a b n na)
  then show ?case
    by (metis P_complete UNIV_I f_endo f_valid iter_join lfp_is_el) 
qed
...
lemma kleene_lfp : 
  ...
  assumes P_complete : "is_complete P" 
  and f_valid : "valid_map f" and f_endo : "dom f = P \<and> cod f = P"
  and f_cont : "is_continuous f"
shows "lfp f = sup P { iter f n \<star> bot P | n . n \<in> UNIV }" 
proof -
  let ?U = "{ (iter f n) \<star> bot P | n . n \<in> UNIV }"
  have "is_directed P ?U"
    using P_complete f_cont f_endo f_valid iter_directed by blast 
  moreover have "lfp f \<in> el P"
    using P_complete f_endo f_valid lfp_is_el by blast 
  moreover have "sup P ?U \<in> el P"
    by (metis (no_types, lifting) P_complete calculation(1) complete_equiv_cocomplete is_directed_def sup_el)
  moreover have "le P (lfp f) (sup P ?U)" 
  proof -
    have "f \<star> (sup P ?U) = sup P {f \<star> u | u. u \<in> ?U}"
      using calculation(1) f_cont f_endo is_continuous_def by force
    moreover have "{f \<star> u | u. u \<in> ?U} \<subseteq> ?U" 
    proof 
      fix x
      assume "x \<in> {f \<star> u |u. u \<in> ?U}" 
      then have "\<exists> n . x = iter f n \<star> bot P"
        by (smt (verit, best) P_complete bot_as_inf f_endo f_valid inf_el iter_app mem_Collect_eq subsetI)
      thus "x \<in> ?U"
        by blast 
    qed
    moreover have "le P (sup P {f \<star> u | u. u \<in> ?U}) (sup P ?U)"
      by (smt (verit, best) P_complete Poset.sup_mono \<open>is_directed P {iter f n \<star> Poset.bot P |n. n \<in> UNIV}\<close> calculation(2) complete_equiv_cocomplete dual_order.trans is_directed_def)
    moreover have "le P (f \<star> (meet P (lfp f) (sup P ?U))) (sup P ?U)"
      by (metis (no_types, lifting) P_complete Poset.lfp_lowerbound Poset.lfp_unfold \<open>Poset.lfp f \<in> el P\<close> \<open>Poset.sup P {iter f n \<star> Poset.bot P |n. n \<in> UNIV} \<in> el P\<close> calculation(1) calculation(3) f_endo f_valid inf_le) 
    ultimately show ?thesis
      by (metis (no_types, lifting) P_complete Poset.lfp_lowerbound \<open>Poset.sup P {iter f n \<star> Poset.bot P |n. n \<in> UNIV} \<in> el P\<close> f_endo f_valid)
  qed
moreover have "le P (sup P ?U) (lfp f)"
  proof -
    ...
    thus ?thesis using sup_is_lub [where ?P=P and ?U="?U" and ?z="lfp f"] 
      by (smt (verit, best) P_complete \<open>Poset.lfp f \<in> el P\<close> bot_def complete_equiv_cocomplete empty_subsetI f_endo f_valid iter_el mem_Collect_eq subset_iff sup_el) 
  qed
  ultimately show ?thesis
    by (meson P_complete is_complete_def valid_antisymmetry) 
qed
