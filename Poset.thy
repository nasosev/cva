...
definition app :: "('a, 'b) PosetMap \<Rightarrow> 'a \<Rightarrow> 'b" (infixr "\<cdot>" 997) where
"app f a \<equiv>
  if a \<in> el (dom f)
...
text \<open>
  ...
  \item The function is monotone, i.e., if \<cdot>a \le a'\<cdot> in the domain, then \<cdot>f(a) \le f(a')\<cdot> in the codomain.
  \end{itemize}
\<close>
...
"valid_map f \<equiv>
  let
      ...
      monotone = (\<forall>a a'. a \<in> e_dom \<and> a' \<in> e_dom \<and> le_dom a a' \<longrightarrow> le_cod (f \<cdot> a) (f \<cdot> a'))
...
text \<open>
  ...
  is undefined.
\<close>
definition compose :: "('b, 'c) PosetMap \<Rightarrow> ('a, 'b) PosetMap \<Rightarrow> ('a, 'c) PosetMap" (infixl "\<odot>" 55) where
  "compose g f \<equiv>
  if dom g = cod f
...
"is_surjective f \<equiv> \<forall> b . b \<in> el (cod f) \<longrightarrow> (\<exists> a . a \<in> el (dom f) \<and> f \<cdot> a = b)"
...
"is_injective f \<equiv> \<forall>a a' . a \<in> el (dom f) \<longrightarrow> a' \<in> el (dom f) \<longrightarrow> f \<cdot> a = f \<cdot> a' \<longrightarrow> a = a'"
...
"direct_image f \<equiv> \<lparr>
        ...
        func = {(p, {f \<cdot>\<cdot> x | x . x \<in> p}) | p . p \<subseteq> Function.dom f}
 \<rparr>"
...
lemma valid_mapI: "valid (dom f) \<Longrightarrow> valid (cod f)  \<Longrightarrow> (\<And>a b. (a, b) \<in> func f \<Longrightarrow>  a \<in> el (dom f) \<and> b \<in> el (cod f)) \<Longrightarrow>
                   ...
                   (\<And>a a'. a \<in> el (dom f) \<Longrightarrow> a' \<in> el (dom f) \<Longrightarrow> le (dom f) a a' \<Longrightarrow> le (cod f) (f \<cdot> a) (f \<cdot> a'))
...
lemma valid_map_monotone : "valid_map f \<Longrightarrow> a \<in> el (dom f) \<Longrightarrow> a' \<in> el (dom f) \<Longrightarrow> le (dom f) a a' \<Longrightarrow> le (cod f) (f \<cdot> a) (f \<cdot> a')"
unfolding valid_map_def
  apply auto
  apply meson
   apply metis
...
text \<open>
   ...
   the pair `(a, f \\<cdot>\\<cdot> a)` is an element of the function `f`. This is essentially a representation
   of the functionality of a poset map.
\<close>
lemma fun_app : "valid_map f \<Longrightarrow> a \<in> el (dom f) \<Longrightarrow> (a, f \<cdot> a) \<in> func f"
  by (metis app_def the_equality valid_map_deterministic valid_map_total)

text \<open>
   This lemma establishes that if `fa` is equivalent to `f \\<cdot>\\<cdot> a` for a valid poset map `f` and an element
   `a` in the domain of `f`, then `fa` is an element of the codomain of `f`.
\<close>
lemma fun_app2 : "valid_map f \<Longrightarrow> a \<in> el (dom f) \<Longrightarrow> fa = f \<cdot> a \<Longrightarrow> fa \<in> el (cod f)"
  by (meson fun_app valid_map_welldefined)

lemma fun_app3 [simp] : "valid_map f \<Longrightarrow> a \<in> el (dom f) \<Longrightarrow> f \<cdot> a = (THE b. (a, b) \<in> func f) "
  by (simp add: app_def)
...
\<close>
lemma fun_ext : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> dom f = dom g \<Longrightarrow> cod f = cod g \<Longrightarrow> (\<forall>a \<in> el (dom f). f \<cdot> a = g \<cdot> a) \<Longrightarrow> f = g"
...
text \<open>
   ...
   the codomain of `f`, the domain of the composition \<cdot>g \cdot f\<cdot> is the same as the domain of `f`.
\<close>
lemma dom_compose [simp] : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> dom g = cod f \<Longrightarrow> dom (g \<odot> f) = dom f"
  unfolding compose_def
  by (simp add: Let_def)

text \<open>
   This lemma states that for two valid poset maps `f` and `g`, where the domain of `g` is
   the codomain of `f`, the codomain of the composition \<cdot>g \cdot f\<cdot> is the same as the codomain of `g`.
\<close>
lemma cod_compose [simp] : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> dom g = cod f \<Longrightarrow> cod (g \<odot> f) = cod g"
  unfolding compose_def
  by (simp add: Let_def)

text \<open>
   This lemma defines the well-definedness of composition of two valid poset maps `f` and `g`.
   If the pair `(a, b)` is in the function of \<cdot>g \cdot f\<cdot>, then `a` is an element of the domain of `f`
   and `b` is an element of the codomain of `g`.
\<close>
lemma compose_welldefined : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> dom g = cod f \<Longrightarrow> (a, b) \<in> func (g \<odot> f) \<Longrightarrow> a \<in> el (dom f) \<and> b \<in> el (cod g)"
...
text \<open>
   ...
   If `(a, b)` and `(a, b')` are both elements of the function \<cdot>g \cdot f\<cdot>, then `b` equals `b'`.
\<close>
lemma compose_deterministic : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> dom g = cod f \<Longrightarrow> (a, b) \<in> func (g \<odot> f) \<Longrightarrow> (a, b') \<in> func (g \<odot> f) \<Longrightarrow> b = b'"
...
text \<open>
   ...
   `(a, b)` is in the function \<cdot>g \cdot f\<cdot>.
\<close>
lemma compose_total : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> dom g = cod f \<Longrightarrow> a \<in> el (dom f) \<Longrightarrow> \<exists>b. (a, b) \<in> func (g \<odot> f)"
  unfolding compose_def                                                                 
  by (smt (z3) Poset.fun_app Poset.fun_app2 PosetMap.select_convs(3) relcomp.relcompI) 

text \<open>
   This lemma states that if `(a, b)` is an element of a valid poset map `f`, then `f \\<cdot>\\<cdot> a` is equivalent to `b`.
\<close>
...
text \<open>
   ...
   of `f` and the domain of `g` is the codomain of `f`, the value of \<cdot>(g \cdot f) \\<cdot>\\<cdot> a\<cdot> is the same as \<cdot>g \\<cdot>\\<cdot> (f \\<cdot>\\<cdot> a)\<cdot>.
\<close>
lemma compose_app: "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> a \<in> el (dom f) \<Longrightarrow> dom g = cod f \<Longrightarrow> (g \<odot> f) \<cdot> a = g \<cdot> (f \<cdot> a)"
...
text \<open>
  ...
  it shows that the application of the composition \<cdot>(g \cdot f)\<cdot> on `a` is less than or equal to the application
...
shows "le (cod g) ((g \<odot> f) \<cdot> a) ((g \<odot> f) \<cdot> a')"
proof -
  have "le (cod f) (f \<cdot> a) (f \<cdot> a')" using valid_map_monotone
    by (metis a'_elem a_elem f_valid le_aa')
  moreover have  "le (cod g) (g \<cdot> (f \<cdot> a)) (g \<cdot> (f \<cdot> a'))" using valid_map_monotone
...
\<close>
lemma compose_valid : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> dom g = cod f \<Longrightarrow> valid_map (g \<odot> f)"
...
lemma comp_app [simp] : "valid_map f \<Longrightarrow> valid_map g \<Longrightarrow> (a, b) \<in> func f \<Longrightarrow> dom g = cod f \<Longrightarrow>
                (b, c) \<in> func g \<Longrightarrow> (g \<odot> f) \<cdot> a = c"
...
\<close>
lemma ident_right_neutral [simp] : "valid_map f \<Longrightarrow> dom f = x \<Longrightarrow> f \<odot> (ident x) = f"
...
\<close>
lemma ident_left_neutral [simp]  : "valid_map f \<Longrightarrow> cod f = x \<Longrightarrow> (ident x) \<odot> f = f"
  unfolding compose_def ident_def
   apply (simp add: Let_def)
  apply safe
  apply (rule pom_eqI; clarsimp?)
...
lemma const_app [simp] : "valid P \<Longrightarrow> valid Q \<Longrightarrow> p \<in> el P \<Longrightarrow> q \<in> el Q \<Longrightarrow> ((const P Q q) \<cdot> p) = q"
...
proof (rule valid_mapI)
 ...
 (const P Q q)"
  ...
  show "\<And>a a'.
       ...
       le (PosetMap.cod (Poset.const P Q q)) (Poset.const P Q q \<cdot> a) (Poset.const P Q q \<cdot> a')"
...
text \<open>
  ...
  `h` are valid maps such that `dom g = cod f` and `dom h = cod f`, and `g \<odot> f = h \<odot> f`, then `g = h`.
\<close>
lemma surjection_is_right_cancellative : "valid_map f \<Longrightarrow> is_surjective f \<Longrightarrow>
  valid_map g \<Longrightarrow> valid_map h \<Longrightarrow> cod f = dom g \<Longrightarrow> cod f = dom h \<Longrightarrow>  g \<odot> f = h \<odot> f \<Longrightarrow> g = h"
...
text \<open>
  ...
  `h` are valid maps such that `cod g = dom f` and `cod h = dom f`, and \<cdot>`f \<odot> g = f \<odot> h`\<cdot>, then `g = h`.
\<close>
lemma injection_is_left_cancellative : "valid_map f \<Longrightarrow> is_injective f \<Longrightarrow>
  valid_map g \<Longrightarrow> valid_map h \<Longrightarrow> cod g = dom f \<Longrightarrow> cod h = dom f \<Longrightarrow>  f \<odot> g = f \<odot> h \<Longrightarrow> g = h"
...
lemma direct_image_app : "Function.valid_map f \<Longrightarrow> a \<subseteq> Function.dom f \<Longrightarrow> (direct_image f) \<cdot> a = {f \<cdot>\<cdot> x | x . x \<in> a}"
...
lemma direct_image_mono_raw: "Function.valid_map f \<Longrightarrow> a \<subseteq> Function.dom f \<Longrightarrow> a' \<subseteq> Function.dom f
 \<Longrightarrow> a \<subseteq> a' \<Longrightarrow> (direct_image f) \<cdot> a \<subseteq> (direct_image f) \<cdot> a'"  
  unfolding Function.valid_map_def app_def direct_image_def
  apply (simp add: Let_def)
...
proof (rule valid_mapI, safe)
 ...
 f)" 
  ...
  show "\<And>a a'.
       ...
       le (PosetMap.cod (direct_image f)) (direct_image f \<cdot> a) (direct_image f \<cdot> a')"
  proof -
    fix a a'
    ...
    have "(direct_image f) \<cdot> a = {f \<cdot>\<cdot> x | x . x \<in> a}" using direct_image_def [where ?f=f] app_def
    ...
    moreover have "(direct_image f) \<cdot> a' = {f \<cdot>\<cdot> x | x . x \<in> a'}" using direct_image_def [where ?f=f] app_def
        [where ?f="direct_image f" and ?a=a']
      using \<open>\<And>a. a \<in> el (PosetMap.dom (direct_image f)) \<Longrightarrow> \<exists>b. (a, b) \<in> PosetMap.func (direct_image f)\<close> \<open>a' \<in> el (PosetMap.dom (direct_image f))\<close> by auto 
    moreover have "{f \<cdot>\<cdot> x | x . x \<in> a} \<subseteq> {f \<cdot>\<cdot> x | x . x \<in> a'}" using direct_image_def [where ?f=f]
        calculation
    ...
    ultimately show "le (PosetMap.cod (direct_image f)) (direct_image f \<cdot> a) (direct_image f \<cdot>
...
shows "le (cod pf) (pf \<cdot> a) (pf \<cdot> a')"
...
proof -
  ...
  moreover have "\<forall> p . p \<subseteq> X \<longrightarrow> {Function.ident X \<cdot>\<cdot> x |x. x \<in> p} = p" using Function.ident_app [where
        ?X=X]
...
shows "direct_image g \<odot> direct_image f = direct_image (g \<odot>\<odot> f)"
proof (rule fun_ext)
    show "Poset.valid_map (direct_image g \<odot> direct_image f)"
      by (simp add: Poset.compose_valid assms(3) direct_image_cod direct_image_dom direct_image_valid f_valid g_valid)
    show "Poset.valid_map (direct_image (g \<odot>\<odot> f))"
      using Function.compose_valid assms(3) direct_image_valid f_valid g_valid by blast
    show "PosetMap.dom (direct_image g \<odot> direct_image f) = PosetMap.dom (direct_image (g \<odot>\<odot> f))"
      by (simp add: assms(3) direct_image_cod direct_image_dom direct_image_valid f_valid g_valid) 
    show "PosetMap.cod (direct_image g \<odot> direct_image f) = PosetMap.cod (direct_image (g \<odot>\<odot> f))"
      by (simp add: assms(3) direct_image_cod direct_image_dom direct_image_valid f_valid g_valid) 
    show "\<forall>a\<in>el (PosetMap.dom (direct_image g \<odot> direct_image f)).
       (direct_image g \<odot> direct_image f) \<cdot> a = direct_image (g \<odot>\<odot> f) \<cdot> a "
    proof 
    fix p
    assume "p \<in> el (PosetMap.dom (direct_image g \<odot> direct_image f))"
    have "p \<subseteq> Function.dom f"
      by (metis (no_types, lifting) Function.dom_compose Poset.Poset.select_convs(1) PowD \<open>PosetMap.dom (direct_image g \<odot> direct_image f) = PosetMap.dom (direct_image (g \<odot>\<odot> f))\<close> \<open>p \<in> el (PosetMap.dom (direct_image g \<odot> direct_image f))\<close> assms(3) direct_image_dom f_valid g_valid powerset_def) 
    have "(p, {f \<cdot>\<cdot> x |x. x \<in> p}) \<in> {(p, {f \<cdot>\<cdot> x |x. x \<in> p}) |p. p \<subseteq> Function.dom f} "
      using \<open>p \<subseteq> Function.dom f\<close> by blast 
    moreover have "{f \<cdot>\<cdot> x |x. x \<in> p} \<subseteq> Function.cod f"
      using Function.fun_app2 \<open>p \<subseteq> Function.dom f\<close> f_valid by fastforce
    moreover have "({f \<cdot>\<cdot> x |x. x \<in> p}, {g \<cdot>\<cdot> (f \<cdot>\<cdot> x) |x. x \<in> p}) \<in> {(p, {g \<cdot>\<cdot> x |x. x \<in> p}) |p. p \<subseteq>
      Function.cod f}"
      using calculation(2) by blast 
    moreover have "(p, {g \<cdot>\<cdot> (f \<cdot>\<cdot> x) |x. x \<in> p}) \<in> 
  {(p, {f \<cdot>\<cdot> x |x. x \<in> p}) |p. p \<subseteq> Function.dom f} O {(p, {g \<cdot>\<cdot> x |x. x \<in> p}) |p. p  \<subseteq> Function.cod f}"
      using calculation(1) calculation(3) by auto
    ultimately
    show "(direct_image g \<odot> direct_image f) \<cdot> p = direct_image (g \<odot>\<odot> f) \<cdot> p "
      by (smt (verit) CollectD Collect_cong Function.compose_app Function.compose_valid Function.dom_compose Poset.compose_app \<open>PosetMap.dom (direct_image g \<odot> direct_image f) = PosetMap.dom (direct_image (g \<odot>\<odot> f))\<close> \<open>p \<in> el (PosetMap.dom (direct_image g \<odot> direct_image f))\<close> assms(3) direct_image_app direct_image_cod direct_image_dom direct_image_valid f_valid g_valid prod.inject subset_eq)
  qed
qed
...
proof auto
  ...
  moreover have "\<forall> y \<in> b . (\<exists> x . x \<in> X \<and> f \<cdot>\<cdot> x = y)"
    using X_def Y_def calculation f_surj by auto 
  define "pre" where "pre = (\<lambda> y. (SOME x. (y \<in> b) \<longrightarrow> (f \<cdot>\<cdot> x = y \<and> x \<in> X) ))" 
  moreover have "\<forall> y . (y \<in> b \<longrightarrow> f \<cdot>\<cdot> (pre y) = y)"
    by (smt (verit, best) \<open>\<forall>y\<in>b. \<exists>x. x \<in> X \<and> f \<cdot>\<cdot> x = y\<close> pre_def someI_ex) 
  moreover have "\<forall> y . y \<in> b \<longrightarrow> pre y \<in> X"
    by (smt (verit) \<open>\<forall>y\<in>b. \<exists>x. x \<in> X \<and> f \<cdot>\<cdot> x = y\<close> pre_def someI)   
  ...
  moreover have "\<forall> y . (y \<in> b \<longrightarrow> ( y = f \<cdot>\<cdot>  (pre y)))" using calculation
  by presburger
  moreover have "\<forall> y . (y \<in> b \<longrightarrow> (\<exists> x \<in> a . y = f \<cdot>\<cdot> x))" using calculation
    by blast 
  moreover have "((\<cdot>\<cdot>) f) ` a = b" using a_def pre_def using calculation
    by (smt (verit, ccfv_threshold) image_Collect_subsetI image_iff subsetI subset_antisym)
  show "\<exists>a. a \<in> el (PosetMap.dom (direct_image f)) \<and> direct_image f \<cdot> a = b"
    by (metis Setcompr_eq_image X_def \<open>(\<cdot>\<cdot>) f ` a = b\<close> calculation(5) calculation(6) direct_image_app f_valid) 
  qed   
